...
interface LayoutProps {
  ...
  onNavigate?: (type: ViewType, slug?: string) => void;
}
...
const Layout: React.FC<LayoutProps> = ({ children, onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 selection:bg-blue-100 flex flex-col">
      {/* Premium Header */}
      <header role="banner" className="sticky top-0 z-[100] glass-card border-b border-white/40 shadow-sm transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4">
          <nav role="navigation" aria-label="Main navigation" className="h-16 md:h-20 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 md:gap-8">
              ...
              <div className="hidden md:flex items-center gap-6">
                <a href="#" onClick={handleHomeClick} className="text-xs font-black uppercase tracking-widest text-slate-500 hover:text-blue-600 transition-colors">{t.nav.home}</a>
                <a href="#explore" onClick={handleLibraryClick} className="text-xs font-black uppercase tracking-widest text-slate-500 hover:text-blue-600 transition-colors">{t.nav.nutrition}</a>
                <a href="#contact" onClick={handleContactClick} className="text-xs font-black uppercase tracking-widest text-slate-500 hover:text-blue-600 transition-colors">{t.nav.contact}</a>
              </div>
          ...
          {/* Mobile Secondary Nav - Scrollable strip for reachability */}
          <nav className="md:hidden flex items-center gap-8 overflow-x-auto pb-3 no-scrollbar border-t border-slate-100 pt-3" aria-label="Mobile quick links">
            <a href="#" onClick={handleHomeClick} className="text-[11px] font-black uppercase tracking-widest text-slate-500 shrink-0 px-1">{t.nav.home}</a>
            <a href="#explore" onClick={handleLibraryClick} className="text-[11px] font-black uppercase tracking-widest text-slate-500 shrink-0 px-1">{t.nav.nutrition}</a>
            <a href="#contact" onClick={handleContactClick} className="text-[11px] font-black uppercase tracking-widest text-slate-500 shrink-0 px-1">{t.nav.contact}</a>
          </nav>
        </div>
      </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            ...
            <nav aria-labelledby="footer-links">
              ...
              <ul className="space-y-4 text-sm font-medium text-slate-600">
                ...
                <li><a href="#explore" onClick={handleLibraryClick} className="hover:text-blue-600 transition-colors">Supportive Care Library</a></li>
